...
async function walk(dir, extensions, fileList = []) {
  const files = await fs.readdir(dir, { withFileTypes: true });
  for (const file of files) {
    const filePath = path.resolve(dir, file.name);
    if (file.isDirectory()) {
      console.log(`${source_default.bgBlue.white(" Traversing ")} Walk into directory ${source_default.underline.yellow(filePath)}`);
      fileList = await walk(filePath, extensions, fileList);
      console.log(`${source_default.green(String("-").repeat(40))}`);
    } else if (extensions.length === 0 || extensions.includes(path.extname(file.name))) {
      console.log(`${source_default.bgBlue.white(" Traversing ")} Add file ${source_default.underline.yellow(file.name)} to process list`);
      fileList.push(filePath);
    }
  }
  return fileList;
}
async function extractTranslationKeys(pattern, directory, extensions) {
  console.log(`${source_default.white.bgBlue(" Setting ")} Use RegExp ${source_default.underline.yellow(pattern)} to match`);
  console.log(`${source_default.green(String("-").repeat(40))}`);
  const keys = [];
  const files = await walk(directory, extensions);
  for (const filePath of files) {
    const content = await fs.readFile(filePath, "utf-8");
    console.log(`${source_default.white.bgBlue(" Processing ")} Scan keys from ${source_default.underline.yellow(filePath)}`);
    const regex = pattern;
    let readText;
    ...
    if (currentFindKeys.length > 0) {
      console.log(`${source_default.white.bgGreen(" Extracting ")} Find key: ${currentFindKeys.map((key) => source_default.underline.yellow(key)).join(", ")}`);
    }
    console.log(`${source_default.green(String("-").repeat(40))}`);
  }
  return keys;
}
function transformArrayToObject(keys) {
...
function isDirectoryExists(directory) {
  try {
    fs2.access(directory, fs2.constants.F_OK | fs2.constants.R_OK | fs2.constants.W_OK, (err) => {
      if (err) {
        return false;
      }
    });
...
(async () => {
  ...
  const { pattern, directories, fileExtType, specifiedFileExt, outputPath } = await import_prompts.default([
    {
      type: "text",
      name: "pattern",
      message: `Enter the pattern to search for, use ${source_default.yellow.underline("%key%")} to match variable`,
      initial: `t('%key%')`,
      validate: (value) => {
      ...
      format: (value) => createRegexFromTemplate(value)
    },
    {
      type: "multiselect",
      name: "directories",
      ...
      choices: subdirectories.map((subDir) => ({
        title: subDir,
        value: path2.join(currentDirectory, subDir),
    ...
    {
      type: (prev) => prev === "specified" ? "autocompleteMultiselect" : null,
    ...
    },
    {
      ...
      initial: path2.resolve(currentDirectory, ".output/lang.json"),
      validate: (value) => {
        if (!value || value.length === 0) {
          return false;
        }
  ...
  if (directories?.length > 0) {
    try {
      ...
      for (const directoryPath of directories) {
        const keys = await extractTranslationKeys(pattern, directoryPath, fileExtType === "specified" ? specifiedFileExt : []);
        allKeys = allKeys.concat(keys);
      }
      const sortedKeys = allKeys.sort();
      console.log(`${source_default.bgGreen.white(" Resulting ")} Found ${source_default.underline.yellow(sortedKeys.length)} keys in total`);
      const objectContent = transformArrayToObject(sortedKeys);
      try {
        if (!isDirectoryExists(path2.dirname(outputPath))) {
          fs3.mkdirSync(path2.dirname(outputPath), { recursive: true });
        }
        fs3.writeFileSync(outputPath, JSON.stringify(objectContent, null, 2));
        console.log(`${source_default.bgGreen.white(" Saving ")} Extracted keys saved to ${source_default.underline.yellow(outputPath)}`);
      } catch (error) {
        console.error(`${source_default.bgRed.white(" Error ")} Writing extracted keys failed:`, error);
      }
    } catch (error) {
      console.error(`${source_default.bgRed.white(" Error ")} Extracting keys failed:`, error);
    }
  }
})();
